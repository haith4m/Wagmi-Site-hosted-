...
interface GalleryModerationProps {
  id: string;
  status: ContentStatus;
}

export function GalleryModeration({ id, status }: GalleryModerationProps) {
  ...
  async function remove() {
    if (!window.confirm("Remove this gallery item permanently?")) return;
    setBusy(true);
    setError(null);
    try {
      const res = await fetch(`/api/gallery/${id}`, { method: "DELETE" });
      if (!res.ok) setError("Could not remove the item.");
      router.refresh();
    } catch {
      setError("Network error.");
    } finally {
